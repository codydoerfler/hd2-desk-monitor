...
> The image above is generated by `tools/preview_hud.py`, which rasterises the
> HUD off-target using the real font glyph data. See
...
Two Python scripts in `tools/` let you check layout changes without flashing.
Both parse the **actual** GFX font tables out of the downloaded TFT_eSPI
library, so their measurements match the device to the pixel. Run them from the
...
every faction badge width, each percentage label) and prints
