...
// Set by poll() when a new Major Order arrives, fired by loop() after the
// repaint that puts that order on screen. See the note at the assignment.
...
// High Command issuing a new Major Order. The card's clip wins when there is
// one, because it is specific to this event where the compiled-in line is the
// generic "go look at the screen" -- the point of the SD card is that events
// stop sounding alike. playClip() returns false for no card, no file, or a
// file this reader will not play, and all three land on the same fallback.
static void playOrderAlert() {
  if (!storage::playClip(storage::kClipNewOrder)) playCompiledAlert();
}
...
static void poll() {
  ...
  // Queued rather than played here: poll() runs before the frame that draws
  // this order, and an alert that finishes while the panel still shows the
  // previous one has told you to look at the wrong thing. loop() fires it
  // once the repaint has landed.
  if (orderIsNew) {
    Serial.printf("[audio] new Major Order %d — \"%s\"\n", (int)order.id,
                  order.title.c_str());
    alertPending = true;
  }
}
